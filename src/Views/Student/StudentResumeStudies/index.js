...
import { getStudentResumeStudies, deleteStudentResumeStudiesRequest, updateStudentResumeStudiesProgress } from '../../../Data/Student';
            ...
            if(record.hod === "approved" && record.principal === "approved" && record.dvc === "approved"){
                return (
                    <center>
                        ...
                        <GenerateResumeStudiesPDF resumeStudiesId={record._id}/>
                    </center>
                );
            }else{
                return (
...
const StudentResumeStudies = () => {
  ...
  const description = [
    ...
    useEffect(() => {
        const fetchResumeStudies = async () => {
            ...
            getStudentResumeStudies(
                localStorage.getItem("token"),
                (data) => {
                    setResumeStudies(data.resumeStudiesrequests);
                    setIsLoading(false);
                    //console.log(data.resumeStudiesrequests);
                },
                (error) => {
                    setIsLoading(false);
                    console.log(error);
    ...
    return (
        <Card
            title="My Resume of Studies" 
            bordered={true} 
            extra={
                <Link to="/applyresumestudies" style={{"float":"right"}}>
                    {
                    ...
                    <center>
                        {
                            ...
                            <Result
                                icon={<SmileOutlined style={{color: "red"}}/>}
                                title="You have never applied for a Resume of Studies!"
                                extra={
                                    <Link to="/applyresumestudies">
