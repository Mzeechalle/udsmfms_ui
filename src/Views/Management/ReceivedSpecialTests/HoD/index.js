...
import { getHoDAssignedStudentSpecialTests, deleteStudentSpecialTestRequest } from '../../../../Data/Student';
...
const HODReceivedStudentSpecialTests = () => {

    const [ hodStudentSpecialTests, setHoDAssignedSpecialTests ] = useState([]);
    const [ isLoading, setIsLoading ] = useState(false);

    useEffect(() => {
        const fetchSpecialTests = async () => {
            setIsLoading(true);
            getHoDAssignedStudentSpecialTests(
                (data) => {
                    setHoDAssignedSpecialTests(data.submitted);
                    setIsLoading(false);
                    //console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ?
                hodStudentSpecialTests.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={hodStudentSpecialTests}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default HODReceivedStudentSpecialTests;