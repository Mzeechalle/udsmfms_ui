...
import { getMoAssignedStudentSpecialTests, deleteStudentSpecialTestRequest } from '../../../../Data/Student';
...
const MoReceivedStudentSpecialTests = () => {

    const [ moStudentSpecialTests, setMoAssignedSpecialTests ] = useState([]);
    const [ isLoading, setIsLoading ] = useState(false);

    useEffect(() => {
        const fetchSpecialTests = async () => {
            setIsLoading(true);
            getMoAssignedStudentSpecialTests(
                (data) => {
                    setMoAssignedSpecialTests(data.submitted);
                    setIsLoading(false);
                    //console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ?
                moStudentSpecialTests.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={moStudentSpecialTests}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default MoReceivedStudentSpecialTests;