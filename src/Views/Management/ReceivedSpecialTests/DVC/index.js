...
import { getHoDAssignedStudentSpecialTests, getApprovedStudentSpecialTests, deleteStudentSpecialTestRequest } from '../../../../Data/Student';
...
const columns = [
...

    {
        ...
        render: (text, record) => (
            <center>
                <Link to="/viewspecialtest" onClick={() => localStorage.setItem("specialtest_id", record._id)}>
...
const DVCReceivedStudentSpecialTests = () => {

    const [ approvedSpecialTests, setDVCAssignedSpecialTests ] = useState([]);
    ...
    useEffect(() => {
        const fetchSpecialTests = async () => {
            ...
            getApprovedStudentSpecialTests(
                "principal",
                (data) => {
                    setDVCAssignedSpecialTests(data.submitted);
                    setIsLoading(false);
                    console.log(data);
                },
                (error) => {
    ...
    return(
        <Card
            title="Submitted SpecialTests" 
            bordered={true}
        >
            {
                !isLoading ?
                    approvedSpecialTests.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={approvedSpecialTests}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default DVCReceivedStudentSpecialTests;