...
import { getMoAssignedStudentSpecialExams, deleteStudentSpecialExamRequest } from '../../../../Data/Student';
...
const MoReceivedStudentSpecialExams = () => {

    const [ moStudentSpecialExams, setMoAssignedSpecialExams ] = useState([]);
    const [ isLoading, setIsLoading ] = useState(false);

    useEffect(() => {
        const fetchSpecialExams = async () => {
            setIsLoading(true);
            getMoAssignedStudentSpecialExams(
                (data) => {
                    setMoAssignedSpecialExams(data.submitted);
                    setIsLoading(false);
                    //console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ?
                moStudentSpecialExams.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={moStudentSpecialExams}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default MoReceivedStudentSpecialExams;