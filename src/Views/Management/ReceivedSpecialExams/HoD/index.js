...
import { getHoDAssignedStudentSpecialExams, deleteStudentSpecialExamRequest } from '../../../../Data/Student';
...
const HODReceivedStudentSpecialExams = () => {

    const [ hodStudentSpecialExams, setHoDAssignedSpecialExams ] = useState([]);
    const [ isLoading, setIsLoading ] = useState(false);

    useEffect(() => {
        const fetchSpecialExams = async () => {
            setIsLoading(true);
            getHoDAssignedStudentSpecialExams(
                (data) => {
                    setHoDAssignedSpecialExams(data.submitted);
                    setIsLoading(false);
                    //console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ?
                hodStudentSpecialExams.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={hodStudentSpecialExams}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default HODReceivedStudentSpecialExams;