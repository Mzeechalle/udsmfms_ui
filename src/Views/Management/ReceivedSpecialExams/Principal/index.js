...
const columns = [
...

    {
        ...
        render: (text, record) => (
            <center>
                <Link to="/viewspecialexams"> 
                    <EyeOutlined
                     style={{"color":"green","fontSize":"16px", marginRight: 30}}
                     onClick={() => localStorage.setItem("specialexams_id", record._id)}
                     />
                </Link>
                <Popconfirm 
...
const PrincipalReceivedStudentSpecialExams = () => {

    const [ approvedSpecialExams, setPrincipalAssignedSpecialExams ] = useState([]);
    ...
    useEffect(() => {
        const fetchSpecialExams = async () => {
            ...
            getApprovedStudentSpecialExams(
                "hod",
                (data) => {
                    setPrincipalAssignedSpecialExams(data.submitted);
                    setIsLoading(false);
                    console.log(data);
                },
                (error) => {
...
export default PrincipalReceivedStudentSpecialExams;