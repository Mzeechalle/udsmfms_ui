...
import { getHoDAssignedStudentResumeStudies, getApprovedStudentResumeStudies, deleteStudentResumeStudiesRequest } from '../../../../Data/Student';
...
const columns = [
...

    {
        ...
        render: (text, record) => (
            <center>
                <Link to="/viewresumestudies" onClick={() => localStorage.setItem("resumestudies_id", record._id)}>
...
const DVCReceivedStudentResumeStudies = () => {

    const [ approvedResumeStudies, setDVCAssignedResumeStudies ] = useState([]);
    ...
    useEffect(() => {
        const fetchResumeStudies = async () => {
            ...
            getApprovedStudentResumeStudies(
                "principal",
                (data) => {
                    setDVCAssignedResumeStudies(data.submitted);
                    setIsLoading(false);
                    console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ?
                    approvedResumeStudies.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={approvedResumeStudies}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default DVCReceivedStudentResumeStudies;