...
import { getHoDAssignedStaffLeaves, deleteStaffLeaveRequest } from '../../../../Data/Staff';
...
const HODReceivedStaffLeaves = () => {

    const [ hodStaffLeaves, setHoDStaffLeaves ] = useState([]);
    const [ isLoading, setIsLoading ] = useState(false);

    useEffect(() => {
        const fetchLeaves = () => {
            setIsLoading(true);
            getHoDAssignedStaffLeaves(
                (data) => {
                    setHoDStaffLeaves(data.submitted);
                    setIsLoading(false);
                    // console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ? 
                    hodStaffLeaves.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={hodStaffLeaves}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default HODReceivedStaffLeaves;