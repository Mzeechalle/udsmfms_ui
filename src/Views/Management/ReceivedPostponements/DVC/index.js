...
import { getHoDAssignedStudentPostponements, getApprovedStudentPostponements, deleteStudentPostponementRequest } from '../../../../Data/Student';
...
const columns = [
...

    {
        ...
        render: (text, record) => (
            <center>
                <Link to="/viewpostponement" onClick={() => localStorage.setItem("postponement_id", record._id)}>
...
const DVCReceivedStudentPostponements = () => {

    const [ approvedPostponements, setDVCAssignedPostponements ] = useState([]);
    ...
    useEffect(() => {
        const fetchPostponements = async () => {
            ...
            getApprovedStudentPostponements(
                "principal",
                (data) => {
                    setDVCAssignedPostponements(data.submitted);
                    setIsLoading(false);
                    console.log(data);
                },
                (error) => {
    ...
    return(
        ...
        >
            {
                !isLoading ?
                    approvedPostponements.length > 0 ?
                    <Table
                        columns={columns}
                        dataSource={approvedPostponements}
                        bordered
                        pagination={{
                        onChange: page => {
                            console.log(page);
...
export default DVCReceivedStudentPostponements;